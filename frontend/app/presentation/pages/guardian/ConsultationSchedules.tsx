...
import {
  useGuardianSchedule,
  useRegisterGuardianSchedule,
} from "../../../features/consultation/application/hooks/useConsultations";
import { useGuardianSeniors } from "../../../features/guardian/application/hooks/useGuardianSeniors";
...
export default function ConsultationSchedules() {
  ...
  const { data: guardianSeniors, isLoading: seniorsLoading } =
    useGuardianSeniors(user?.id as number);
  const [selectedSeniorId, setSelectedSeniorId] = useState<number | undefined>(
    undefined
  );

  // 시니어 목록 로딩이 완료되고 시니어가 아직 선택되지 않았으면 첫 번째 시니어 자동 선택
  // useEffect(() => {
  //   if (selectedSeniorId === undefined && guardianSeniors?.data?.length) {
  //     setSelectedSeniorId(guardianSeniors.data[0].id);
  //   }
  // }, [guardianSeniors, selectedSeniorId]);
  ...
  const { mutate: registerGuardianSchedule } = useRegisterGuardianSchedule();

  // 보호자가 선택한 상담 일정 조회
  const { data: guardianSchedule, isLoading: scheduleLoading } =
    useGuardianSchedule(selectedSeniorId);

  // Mock 데이터를 useMemo로 메모이제이션
  const mockGuardianSchedule = useMemo(
    () => ({
      checkedTime: [
  ...
  const effectiveGuardianSchedule = useMemo(() => {
    return guardianSchedule?.checkedTime &&
      guardianSchedule.checkedTime.length > 0
      ? guardianSchedule
      : mockGuardianSchedule;
  }, [guardianSchedule, mockGuardianSchedule]);

  // 오늘 날짜 기준 +3일 전인지 확인하는 함수
  const isPastDate = (date: Dayjs) => {
    const today = dayjs();
    const threeDaysAgo = today.add(3, "day");
    return date.isBefore(threeDaysAgo, "day");
  };
  ...
  const hasScheduleOnDate = (date: Dayjs) => {
    ...
    // effectiveGuardianSchedule에서 해당 날짜에 시간이 있는지 확인 (전체 저장된 데이터만)
    const hasFromEffective =
      effectiveGuardianSchedule?.checkedTime?.some((time) => {
  ...
  useEffect(() => {
    ...
    if (isPastDate(selectedDate)) {
      ...
      if (hasScheduleOnDate(selectedDate)) {
        const guardianSelectedTimes: string[] = [];
        if (effectiveGuardianSchedule?.checkedTime) {
          effectiveGuardianSchedule.checkedTime.forEach((time) => {
            const date = dayjs(time);
            const timeDate = date.format("YYYY-MM-DD");
            if (timeDate === dateString) {
              const timeStr = date.format("HH:mm");
              guardianSelectedTimes.push(timeStr);
            }
          });
        }
        setSelectedDateSchedules(guardianSelectedTimes);
      } else {
    ...
    // 2. 보호자가 선택한 상담 일정에서 해당 날짜의 시간 확인 (우선순위 2)
    // 단, allSelectedSchedules에 해당 날짜가 저장되어 있으면 effectiveGuardianSchedule은 무시
    const guardianSelectedTimes: string[] = [];
    const hasDateInSaved = allSelectedSchedules.hasOwnProperty(dateString);

    if (!hasDateInSaved && effectiveGuardianSchedule?.checkedTime) {
      effectiveGuardianSchedule.checkedTime.forEach((time) => {
        // ISO 문자열 형식 파싱
        const date = dayjs(time);
        const timeDate = date.format("YYYY-MM-DD");
    ...
    // 3. 저장된 데이터가 있으면 그것을 사용 (빈 배열 포함), 없으면 effectiveGuardianSchedule 사용
    const allTimes = hasDateInSaved ? savedTimes : guardianSelectedTimes;
    setSelectedDateSchedules(allTimes);
  }, [selectedDate, effectiveGuardianSchedule, allSelectedSchedules]);
  ...
  const handleSaveAll = () => {
    if (!selectedSeniorId) {
      message.warning("시니어를 먼저 선택해주세요.");
      return;
    }
    const allCheckedTimes: Date[] = [];

    // 1. 기존에 useGuardianSchedule에서 받아온 데이터 추가 (effectiveGuardianSchedule 사용)
    // 단, 새로 선택한 날짜의 시간은 제외 (나중에 새로 선택한 데이터로 대체)
    if (effectiveGuardianSchedule?.checkedTime) {
      effectiveGuardianSchedule.checkedTime.forEach((timeStr) => {
        // ISO 문자열 형식 파싱
  ...
  const dateCellRender = (value: Dayjs) => {
    ...
    if (hasConsultations) {
      ...
      const bgColor = isPast ? "bg-gray-400" : "bg-blue-500";
    ...
    <AppLayout>
      ...
      >
        ...
        <Row gutter={[24, 24]}>
          ...
          <Col xs={24} lg={8}>
            <Select
              placeholder="예약을 관리할 시니어를 선택해주세요."
              loading={seniorsLoading}
              value={selectedSeniorId}
              onChange={(v) => setSelectedSeniorId(v)}
              size="middle"
              style={{ width: "100%" }}
            >
              {guardianSeniors?.data?.map((senior) => (
                <Option key={senior.id} value={senior.id}>
                  {senior.name}
                </Option>
              ))}
            </Select>
            {typeof selectedSeniorId === "number" && (
              <Card
                title={
                  <div
                    style={{
              ...
              >
                {isPastDate(selectedDate) ? (
                  <Alert
                    message={`오늘로부터 3일 이후의 날짜만 수정/생성 할 수 있습니다.`}
                    type="warning"
                    showIcon
                  ...
                  <Alert
                    message="달력에서 날짜를 선택하면 해당 날짜의 상담 시간을 설정할 수 있습니다."
                    type="info"
                    showIcon
                    style={{ marginBottom: "16px" }}
                    closable
                ...
                >
                  {timeSlots.map((timeSlot) => {
                    ...
                    const isDisabled = isPastDate(selectedDate);
                    return (
                      <AntButton
                        key={timeSlot}
                        size="small"
